<template>
    ...
    <div  class="pro">
      <h1>青岛前港设备状态安全监测及故障诊断平台</h1>
    </div>
    <Form ref="loginForm" autoComplete="off" :model="loginForm" :rules="loginRules"  class="card-box login-form">
        <!-- <Form-item prop="param_act">         
            <Input type="hidden" v-model="loginForm.param_act" placeholder="用户名" autoComplete="on">
            </Input>
        </Form-item>   -->
        <Form-item prop="usercode">
            <Input type="text" v-model="loginForm.usercode" placeholder="用户名" autoComplete="on">
      ...
      methods: {
        handleLogin() {
          this.$refs.loginForm.validate(valid => {
            if (valid) {
              this.loading = true;
              this.$store.dispatch('FoolLogin', this.loginForm).then((res) => {
